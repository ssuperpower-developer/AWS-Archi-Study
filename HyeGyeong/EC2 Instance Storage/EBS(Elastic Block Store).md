...
- **EBS 볼륨을 여러 EC2 인스턴스에 동시에 연결**할 수 있는 기능.
- 이 기능은 **io1과 io2** 제품군에서만 사용 가능하며, 동일한 **가용 영역(AZ)** 내에서만 작동.
- 여러 인스턴스가 하나의 EBS 볼륨에 **동시에 읽고 쓸 수 있다**.

### 제한 사항

- **연결 가능한 EC2 인스턴스의 수는 최대 16개**로 제한. (중요!!)
- **다른 가용 영역(AZ)** 간의 연결 불가. (예: `us-east-1a`에서 `us-east-1b`로 연결 불가)
- **클러스터 인식 파일 시스템**만 지원.
